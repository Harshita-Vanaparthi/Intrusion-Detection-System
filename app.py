...
app = Flask(__name__)
CORS(app)
model = joblib.load('C:\\Users\\Harshita\\IDS\\Public\\IDS.pkl')
...
def train():
    ...
    #return jsonify({'message': 'Model trained successfully', 'data': csv_data})
    # Check if the file part is present in the request

    # Check if the target column 'class' exists in the dataset
    if 'class' not in data.columns:
        app.logger.error('No target column named "class" in the dataset')
        return jsonify({'error': 'No target column named "class" in the dataset'}), 400
    ...
    #joblib.dump((lg_model, scaler, selected_features), model_path)
